...
## Turn 3の禁則：PM判断はAIに書かせない

Turn 3の統合出力のうち、**「PM最終判断」と「判断理由」の2欄はAIに出力させず、空欄の見出しだけ置かせる。** AIが統合案から最終判断まで一気通貫で書くと、PMの仕事が承認ボタンに縮退し、本OSの思想（PMは仮説検証速度を設計する人）と逆行する。AIの仕事は判断材料の整理まで。判断と、その判断への責任はPMが書く。

## 完全独立モード（重案件向けオプション)

デフォルトではTurn 2にTurn 1の出力を参考として見せるが、これは軽度のアンカリング（Turn 1の論点に引っ張られる）を残す。安全・契約・証跡が特に重い案件では、Turn 2にTurn 1を一切見せない**完全独立モード**を使う。独立性は上がるが、Turn 1と論点が噛み合わずTurn 3の統合コストが増えるトレードオフがある。通常案件はデフォルトで十分。

## 自動実行

このプロトコルを手動コピペで回すと30分ゴールが溶ける。`orchestration/run_vdr.py` で3ターンを1コマンド実行できる（完全独立モードは `--independent`）。手動運用は、スクリプトが使えない環境でのフォールバックと位置づける。
